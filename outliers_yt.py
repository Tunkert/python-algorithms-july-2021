...
class Outliers:
    ...
    def lower_median(self):
        ...
        if (math.floor(len(self.data_list) / 2)) % 2 == 0:
            lower_med = (self.data_list[lower_med_index] + self.data_list[lower_med_index - 1]) / 2
        else:
            lower_med = self.data_list[lower_med_index]
        return lower_med

    def upper_median(self):
        if (math.floor(len(data_list) / 2)) % 2 == 0:
            upper_med_index = int(math.ceil(len(self.data_list) * 0.75))
            upper_med = (self.data_list[upper_med_index] + self.data_list[upper_med_index - 1]) / 2
        else:
            upper_med_index = int(math.floor(len(self.data_list) * 0.75))
            upper_med = self.data_list[upper_med_index]
        return upper_med
